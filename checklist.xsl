...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- Output Type Parameter
        ...
        blank:      output a blank document with only default data for hand-completion
        flat:       output a filled document with data and no sections omitted
        compressed: output a filled document with data and sections omitted based ob main checkbox
        fillable:   output a fillable form with all sections and the current data
        -->
    ...
    <xsl:template name="process_form">
        <fo:page-sequence master-reference="page1">
            ...
            <fo:flow flow-name="xsl-region-body" id="end_of_document">
                ...
                <xsl:if test="$output-type='fillable'">
                    <!-- Build Buttons -->
    ...
    <xsl:template match="checkbox">
        <xsl:choose>
            ...
            <xsl:when test="$output-type='flat' or $output-type='compressed'">
                <fo:block font-family="ZapfDingbats">
                    <xsl:choose>
            ...
            <xsl:when test="$output-type='fillable'">
                <fo:block margin-left="10pt" font-size="{$default.font-size} + 2pt">
                    <fo:inline>
        ...
        <xsl:choose>
            <xsl:when test="$output-type='blank'">
                <xsl:call-template name="outputline">
                    ...
                    <xsl:with-param name="totallines">
                        <xsl:value-of select="@printlines"/>
                    </xsl:with-param>
                </xsl:call-template>
            </xsl:when>
            <xsl:when test="$output-type='flat'">
    ...
    <xsl:template name="build_buttons">
        ...
        <fo:block-container text-align="{$button.align}">
            ...
            <fo:block padding-top="2pt">
                <fo:inline>
                    <xsl:for-each select="/form/actions/action">
    ...
    <xsl:template name="show_part">
        <xsl:choose>
            <xsl:when test="$output-type='blank' or $output-type='flat' or $output-type='fillable'">
                <xsl:text>yes</xsl:text>
            </xsl:when>
            <xsl:otherwise>
                <xsl:if test="descendant::checkbox[@value='true']">
                    <xsl:text>yes</xsl:text>
                </xsl:if>
            </xsl:otherwise>
        </xsl:choose>
